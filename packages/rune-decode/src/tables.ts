/** Standard QR tables the decoder needs (mirrors the encoder side). */

export type Ecl = 'L' | 'M' | 'Q' | 'H';

/** ECL by 2-bit format value: M=0, L=1, H=2, Q=3. */
export const ECL_BY_FORMAT: Ecl[] = ['M', 'L', 'H', 'Q'];
...
export function alignmentPatternPositions(version: number): number[] {
  ...
  const result: number[] = [];
  for (let i = 0, pos = version * 4 + 10; i < numAlign - 1; i++, pos -= step) result.unshift(pos);
  result.unshift(6);
  return result;
}
